...
export const ContentSecurityPolicy = `
 default-src 'self';
 connect-src 'self' *;
 script-src 'self' 'unsafe-inline' 'unsafe-eval' https://*.getbeamer.com https://www.googletagmanager.com https://*.ingest.sentry.io https://sentry.io ${
   !IS_PRODUCTION || CYPRESS_MNEMONIC
     ? '' // 不需要额外添加，因为我们已经全局添加了 'unsafe-eval'
     : "'wasm-unsafe-eval'"
 };
